...
async function ensureManagePermission(userId: string, locationId: string) {
  ...
  if (actor.role === 'brand_branch_admin') {
    if (!actor.locationId || actor.locationId !== locationId) {
      throw new Error('Forbidden: Brand branch admin can only manage operating hours for own location');
    }
    return;
  }
  ...
  if (actor.role === 'dealer_admin' || actor.role === 'brand_admin') {
    ...
    if (!actorDealerId || targetLocation.dealer_id !== actorDealerId) {
      throw new Error('Forbidden: Dealer admin can only manage hours for own dealer locations');
    }
    return;
  }
