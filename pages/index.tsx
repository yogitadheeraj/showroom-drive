...
import showcaseBooking from '../src/assets/showcase-booking.jpg';
...
export default function AutoAdvantLandingPage({ initialContent = null }: AutoAdvantLandingPageProps) {
    ...
    const handleDemoRequest = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let customer = await fetchJson(`/api/customers?phone=${encodeURIComponent(demoForm.phone.trim() || 'demo-request')}&limit=1`)
            ...
            <main className={`landing-main bg-background text-foreground${resolvedTheme === 'dark' ? ' dark' : ''}`}>
                ...
                <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                        {[
                            { img: showcaseAdminDashboard.src, title: 'Admin Command Center', desc: 'Live KPIs, pipeline health, and team performance in one unified view.' },
                            { img: showcaseBooking.src, title: 'Test Drive Booking Flow', desc: 'Frictionless scheduling with real-time slot availability and auto-confirmation.' },
                            { img: showcaseGroAssign.src, title: 'GRO Assignment & Handover', desc: 'One-click assignment with key handover tracking and full audit trail.' },
                        ].map((screenshot) => (
...
export async function getServerSideProps() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'data', 'index-content.json');
    const raw = await fs.readFile(filePath, 'utf-8');
    const initialContent = JSON.parse(raw);
    return { props: { initialContent } };
  } catch {
    return { props: { initialContent: null } };
  }
}
